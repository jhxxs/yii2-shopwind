$(function(){
	...
	$.ajax({url: url(['webim/user']), dataType: "json", async:false, success: function(data){
		if(data.done) {
		...
		socket.onopen = function()
		{
			...
			socket.send('socket open');
		  	...
		  	layim.config({
				...
				uploadImage: {
				  url: url(['webim/upload']) //（返回的数据格式见下文）
				  ,type: '' //默认post
				}
				...
				*/
				...
				,chatLog: url(['webim/chatlog']) //聊天记录地址
				//,find: url(['webim/find']) //查找好友/群的地址（如果未填则不显示）
				,copyright: true //是否授权
				,notice: true
		  });
		  ...
		  function chatMessage(res) {
			...
			if(res.id == IM_VISITOR.id || res.id == TOID) {
				...
				$.ajax({url: url(['webim/unread', {logid: res.logid}]), dataType: "json", async:false, success: function(data){
				  if(data.done) {
		  ...
		  if(TOID > 0) {
			  $.ajax({url: url(['webim/user', {toid: TOID}]), dataType: "json", async:false, success: function(data){
				  if(data.done) {
					//跟客服创造一个临时会话
		  	  ...
		  	  // 初始化未读信息量
		  	  $.ajax({url: url(['webim/totalunread']),dataType:"json", async:false, data:{toid: IM_VISITOR.id},success: function(data){
				if(data.done) {
					if(data.retval > 0) {
		  ...
		  $('.J_StartLayim').on('click', function() {
			  location.href = url(['webim/index', {toid:$(this).attr('data-toid')}]);
		  });
		  
		  // iPhone下这种写法失效
		  //$('body').on('click', '.J_Message', function(){
		  $('.J_Message').on('click', function(){
			  location.href = url(['webim/friend']);
		  });
		  ...
		  //layim.on('ready', function(res) {
			...
			layim.on('sendMessage', function(data) {
			   ...
			   //$.ajax({url: url(['webim/forbid', {uid:data.mine.id}]), dataType: "json", async:false, success: function(res){
				  //if(res === 1) {
					//sayDisabled();
				 // }
				  //else
...
// 获取浏览器地址的各个参数，如：index.php?page=1，获取pag的参数值为：getUrlParam('page')
