...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  var elemChatTpl = ['<div class="page-webim layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<div class="layim-chat-title" style="background-color: {{d.base.chatTitleColor}};">'
      ,'<p><span class="layim-back wind-icon-font" onclick="window.history.back();">&#xe628;</span>{{ d.data.name }}<span class="layim-chat-status"></span><span class="layim-message wind-icon-font J_Message">&#xe61a;<ins></ins></span></p>'
    ,'</div>'
    ,'{{# } }}'
  ...
  var layimChat, layimMin, To = {}, popchat = function(data){
    ...
    return popchat.index = layer.open({
      ...
      ,success: function(elem){
        ...
        hotkeySend();
		//viewChatlog();
        viewChatlog(data); // 需要data.id从服务器读取会话记录，by shopwind
  ...
  */
  ...
  //渲染本地最新聊天记录到相应面板（从服务器加载数据，这样才能确保各个终端读取的数据是一致的 by shopwind）
  var viewChatlog = function(data){
    var local = layui.data('layim-mobile')[cache.mine.id] || {};
    var thatChat = thisChat(), chatlog = local.chatlog || {};
    var ul = thatChat.elem.find('.layim-chat-main ul');
	
	$.ajax({ url: url(['webim/lastchat']), dataType: "json", data: {id:data.id,limit:MAX_ITEM}, success: function(data){
		if(data.done) {
