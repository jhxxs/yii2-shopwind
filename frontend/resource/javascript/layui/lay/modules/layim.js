...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(chat[0]){
	  ...
	  listThat[0] || viewChatlog(data); // 需要data.id从服务器读取会话记录，by shopwind
      setHistory(data);
      hotkeySend();
  ...
  */
  ...
  var viewChatlog = function(data){
	...
	$.ajax({url: url(['webim/lastchat', {ajax:1}]), dataType: "json", data: {id:data.id,limit:MAX_ITEM}, success: function(data){
		if(data.done) {
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
		...
		,content: cache.base.chatLog + '&id=' + thatChat.data.id + '&type=' + thatChat.data.type //  by shopwind
      });
    }
