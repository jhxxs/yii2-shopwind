	...
	layui.use(['layim', 'layer', 'jquery'], function(layim) {
		...
		socket.onopen = function()
		{
			socket.send('socket open');
		  	...
		  	layim.config({
				...
				*/
				...
				,chatLog: url(['webim/chatlog', {ajax: 1}]) //聊天记录地址
		  ...
		  $('.J_StartLayim').on('click', function(){
			  ...
			  $.ajax({url: url(['webim/user']), dataType: "json", async:false, data: {toid: $(this).attr('data-toid')}, success: function(data){
		  ...
		  layim.on('ready', function(res) {
			...
			layim.on('sendMessage', function(data) {
			   ...
			   //$.ajax({url: url(['webim/imforbid']), dataType: "json", async:false, data: {uid: data.mine.id}, success: function(res){
				  //if(res === 1) {
					//sayDisabled();
				 // }
				  //else
