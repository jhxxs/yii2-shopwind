...
use common\plugins\payment\wxmppay\lib\Mp_pub;
use common\plugins\payment\wxmppay\lib\UnifiedOrder_pub;
use common\plugins\payment\wxmppay\lib\Notify_pub;
...
class SDK
{
	...
	public function getPayform($orderInfo = array())
	{
		...
		$unifiedOrder->setParameter("openid", "oTdYI0agBXXBwfSSavcKRt2fv59Y");
		...
		// 返回给APP客户端参数
		$client = new Mp_pub($this->config);
		$prepay_id = $unifiedOrderResult['prepay_id'];
		$client->setPrepayId($prepay_id);
		$client->setPartnerId($unifiedOrderResult['mch_id']);
