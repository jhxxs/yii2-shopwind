...
class NormalOrder extends BaseOrder
{
	...
	public function formData(&$goods_info = array())
	{
		...
		// 当结算的商品类型有实物结算时
		if (in_array('material', $goods_info['gtype'])) {
	...
	public function submit($data = [])
	{
		...
		// 处理订单收货人信息
		if (($consignee_info = $this->handleConsigneeInfo($goods_info)) === false) {
			return false;
		}
	...
	public function getOrderGoodsList()
	{
		$query = CartModel::find()->alias('c')
			->select('gs.spec_id,gs.spec_1,gs.spec_2,gs.stock,c.rec_id,c.userid,c.store_id,c.goods_id,c.goods_name,c.goods_image,c.gtype,c.specification,c.price,c.quantity,c.product_id,c.selected')
