...
class GuidebuyOrder extends NormalOrder
{
	...
	public function getOrderGoodsList()
	{
		...
		{
			if(($goods = GoodsSpecModel::find()->alias('gs')->select('gs.spec_id,gs.price,gs.spec_1,gs.spec_2,gs.stock,gs.image,g.goods_id,g.type as gtype,g.store_id,g.type as gtype,g.goods_name,g.default_image as goods_image,g.spec_name_1,g.spec_name_2')->where(['spec_id' => $value->spec_id])->joinWith('goods g', false)->asArray()->one())) 
			{
				// 读取商品促销价格
