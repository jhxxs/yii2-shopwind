...
class Taskqueue
{
	...
	private static function getConditions($query = null, $otype = 'normal')
	{
		...
		if (isset(Yii::$app->params['autodeliveryday']) && ($autodeliveryday = intval(Yii::$app->params['autodeliveryday']))) {
			$deliveryday = $autodeliveryday;
		}

		$today = Timezone::gmtime();
		$interval = ($otype == 'guidebuy' ? $deliveryday : $shipday) * 24 * 3600;
