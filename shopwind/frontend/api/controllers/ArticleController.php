...
class ArticleController extends \common\base\BaseApiController
{
	...
	public function actionRead()
	{
		...
		// 业务参数
		$post = Basewind::trimAll($respond->getParams(), true, ['article_id']);

		$record = ArticleModel::find()->select('article_id, title, description, add_time')->where(['article_id' => $post->article_id, 'if_show' => 1])->asArray()->one();
		if ($record) {
