...
class DepositController extends \common\base\BaseApiController
{
	...
	public function actionRead()
	{
		...
		$record = UserModel::find()->select('userid,username')->where(['userid' => Yii::$app->user->id])
			->asArray()->one();

		foreach ($query as $key => $value) {
			if (in_array($key, ['account', 'money', 'nodrawal', 'frozen', 'pay_status'])) {
	...
	public function actionRecordlist()
	{
		...
		// 获取指定的时间段的订单
		if ($post->begin) {
			$query->andWhere(['>=', 'pay_time', Timezone::gmstr2time($post->begin)]);
		}
		if ($post->end) {
			$query->andWhere(['<=', 'pay_time', Timezone::gmstr2time($post->end)]);
		}
