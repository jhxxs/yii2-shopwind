...
class GoodsController extends \common\base\BaseApiController
{
	...
	public function actionList()
	{
		...
		foreach ($list as $key => $value) {
			...
			// 该商品是否为社区团购商品
			if ($result = GuideshopModel::getCategoryId(true)) {
				if ($result && in_array($value['cate_id'], $result)) {
					$list[$key]['activity'][] = 'guidebuy';
				}
			}
	...
	public function actionSearch()
	{
		...
		$query = GoodsModel::find()->alias('g')
			->select('g.goods_id,g.goods_name,g.cate_id,g.brand,g.if_show,g.closed,g.add_time,g.default_spec,g.default_image,g.long_image,g.recommended,g.price,g.mkprice,g.dt_id,s.store_id,s.store_name,gs.stock,gst.views,gst.collects,gst.sales,gst.comments')
			->joinWith('goodsStatistics gst', false)
			->where(['s.state' => 1]);
	...
	public function actionAdd()
	{
		...
		// 业务参数
		$post = Basewind::trimAll($respond->getParams(), true, ['recommended', 'if_show', 'isnew', 'cate_id', 'spec_qty', 'dt_id']);
	...
	public function actionSpecs()
	{
		...
		$query = GoodsSpecModel::find()->alias('gs')
			->select('gs.goods_id,gs.spec_id,gs.spec_1,gs.spec_2,gs.price,gs.mkprice,gs.stock,gs.image,gst.sales,g.goods_name,g.default_image,g.spec_qty,g.spec_name_1,g.spec_name_2')
	...
	public function actionSpec()
	{
		...
		$record = GoodsSpecModel::find()->select('spec_id,goods_id,spec_1,spec_2,price,mkprice,stock,image')->where(['spec_id' => $post->spec_id])->asArray()->one();
		if (!$record) {
	...
	public function actionSalelogs()
	{
		...
		$query = OrderGoodsModel::find()->alias('og')->select('og.rec_id,og.spec_id,og.goods_id,og.goods_name,og.goods_image,og.specification,og.price,og.quantity,og.evaluation,o.evaluation_status,o.buyer_id,o.buyer_name,o.add_time,o.anonymous')
	...
	public function actionComments()
	{
		...
		$query = OrderGoodsModel::find()->alias('og')
			->select('og.rec_id,og.spec_id,og.goods_id,og.goods_name,og.specification,og.evaluation,og.comment,og.reply_comment,og.reply_time,og.images,o.buyer_id,o.buyer_name,o.evaluation_time')
			->joinWith('order o', false)
			->where(['goods_id' => $post->goods_id, 'o.evaluation_status' => 1])
	...
	private function getGuideshop($post)
	{
		...
		return [$model->name, Formatter::path($model->banner, 'store'), Resource::getResourceUrl('system/guideGoodsQrcode.jpg')];
	}
}
