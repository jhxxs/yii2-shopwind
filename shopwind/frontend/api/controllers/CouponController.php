...
class CouponController extends \common\base\BaseApiController
{
    ...
    public function actionUpdate()
    {
		...
		// 业务参数
		$post = Basewind::trimAll($respond->getParams(), true, ['coupon_id', 'total']);

		$model = new \frontend\home\models\Seller_couponForm(['store_id' => Yii::$app->user->id, 'coupon_id' => $post->coupon_id]);
   		if(($record = $model->save($post, true)) === false) {
			return $respond->output(Respond::HANDLE_INVALID, $model->errors);
		}
    ...
    public function actionDelete()
    {
		...
		// 删除后把用户领取的优惠券号也删除
		CouponsnModel::deleteAll(['coupon_id' => $post->coupon_id]);
