...
class WeixinController extends \common\controllers\BaseAdminController
{
	...
	{
		if (!Yii::$app->request->isPost) {
			$applet = WeixinSettingModel::find()->where(['userid' => 0, 'code' => 'applet'])->orderBy(['id' => SORT_DESC])->asArray()->one();
			$this->params['applet'] = $applet;

			if ($merchant = WeixinSettingModel::find()->where(['userid' => 0, 'code' => 'merapplet'])->orderBy(['id' => SORT_DESC])->asArray()->one()) {
